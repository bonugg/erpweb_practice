...
public interface MeetingRepository extends JpaRepository<Meeting, Long> {
    ...
    @Query("SELECT T, U.CNO, U.NAME FROM Meeting T JOIN FETCH User U ON T.user = U WHERE U.DEPT =:DEPT ORDER BY T.VNO DESC")
    List<Object[]> findByDeptList(@Param("DEPT") String DEPT);

    @Query("SELECT T, U.CNO, U.NAME FROM Meeting T JOIN FETCH User U ON T.user = U WHERE U.NO =:NO ORDER BY T.VNO DESC")
